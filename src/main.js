...
const filmsTopRated = getTopRatedFilmsData(filmsData).slice(
  0,
  FILM_EXTRA_QUANTITY
);
const filmsTopCommented = getTopCommentedFilmsData(filmsData).slice(
  0,
  FILM_EXTRA_QUANTITY
);
...
let shownFilmQuantity =
  filmsData.length > FILM_CARD_INITIAL_VALUE
...
renderTemplate(
  sectionHeader,
  getUserProfileTemplate?.(userRank),
  RenderPosition.BEFOREEND
);
renderTemplate(
  sectionMain,
  getMainMenuTemplate?.(filmStatistics),
  RenderPosition.BEFOREEND
);
renderTemplate(
  sectionMain,
  getFilmSortMenuTemplate?.(),
  RenderPosition.BEFOREEND
);
...
if (shownFilmQuantity) {
  ...
  renderTemplate(
    filmSection,
    getFilmListSectionTemplate?.(SectionMessages.DEFAULT),
    RenderPosition.BEFOREEND
  );
  const filmMainListSection = filmSection.querySelector(
  ...
  if (filmsTopRated.length >= FILM_EXTRA_QUANTITY) {
    renderTemplate(
      filmSection,
      getFilmListSectionTemplate?.(SectionMessages.RATED, FILM_EXTRA_SECTION),
      RenderPosition.BEFOREEND
    );
    filmRatedListSection = filmSection.querySelector(
  ...
  if (filmsTopCommented.length >= FILM_EXTRA_QUANTITY) {
    renderTemplate(
      filmSection,
      getFilmListSectionTemplate?.(SectionMessages.POPULAR, FILM_EXTRA_SECTION),
      RenderPosition.BEFOREEND
    );
    filmPopularListSection = filmSection.querySelector(
      ".films-list--extra:last-child .films-list__container"
    );
  }
  ...
  for (let i = 0; i < shownFilmQuantity; i++) {
    renderTemplate(
      filmMainListSection,
      getFilmCardTemplate?.(filmsData[i]),
      RenderPosition.BEFOREEND
    );
    ...
    if (i < FILM_EXTRA_QUANTITY) {
      if (filmRatedListSection) {
        renderTemplate(
          filmRatedListSection,
          getFilmCardTemplate?.(filmsTopRated[i]),
          RenderPosition.BEFOREEND
        );
      }
      ...
      if (filmPopularListSection) {
        renderTemplate(
          filmPopularListSection,
          getFilmCardTemplate?.(filmsTopCommented[i]),
          RenderPosition.BEFOREEND
        );
      }
    }
  ...
  const onShowMoreButtonClick = (evt) => {
    if (shownFilmQuantity < filmsData.length) {
      ...
      shownFilmQuantity +=
        restFilms < FILM_CARD_INITIAL_VALUE
          ? restFilms
          : FILM_CARD_INITIAL_VALUE;
      filmsData
        .slice(lastShownQuantity, shownFilmQuantity)
        .forEach((film) =>
          renderTemplate(
            filmMainListSection,
            getFilmCardTemplate?.(film),
            RenderPosition.BEFOREEND
          )
        );
  ...
  renderTemplate(
    sectionMain,
    getFilmSectionEmptyTemplate?.(SectionMessages.NO_MOVIES),
    RenderPosition.BEFOREEND
  );
}
...
if (shownFilmQuantity) {
  renderTemplate(
    footerStatistic,
    getFilmFooterTemplate?.(filmStatistics),
    RenderPosition.BEFOREEND
  );
  renderTemplate(
    sectionFooter,
    getPopupTemplate?.(filmsData[0]),
    RenderPosition.AFTEREND
  );
  ...
  const popupCommentSelectors = filmPopup.querySelectorAll(
    ".film-details__bottom-container li button"
  );

  const onButtonCloseClick = (evt) => {
    evt.target.closest("li").remove();
  };
  popupCommentSelectors.forEach((commentSelector) =>
    commentSelector.addEventListener("click", onButtonCloseClick)
  );
